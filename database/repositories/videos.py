...
def get_original_by_path(full_path: str) -> dict | None:
    with get_connection() as conn:
        cur = get_cursor(conn)
        cur.execute(
            "SELECT * FROM original_videos WHERE full_path = ?",
            (full_path,)
        )
        return cur.fetchone()
...
def delete_original(full_path: str):
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute("DELETE FROM original_videos WHERE full_path = ?", (full_path,))
        conn.commit()
...
def delete_retargeted(full_path: str):
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute("DELETE FROM retargeted_videos WHERE full_path = ?", (full_path,))
        conn.commit()
