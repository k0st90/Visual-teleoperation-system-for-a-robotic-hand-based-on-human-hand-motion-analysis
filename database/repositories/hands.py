...
def get_by_name(name: str) -> dict | None:
    with get_connection() as conn:
        cur = get_cursor(conn)
        cur.execute(
            "SELECT id, name, yml_path, assets_path FROM hands WHERE name = ?",
            (name,)
        )
        return cur.fetchone()
...
def update_paths(name: str, yml_path: str, assets_path: str):
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute(
            "UPDATE hands SET yml_path = ?, assets_path = ? WHERE name = ?",
...
def delete(name: str):
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute("DELETE FROM hands WHERE name = ?", (name,))
        conn.commit()
